...
func NewPostJournalHandler(
	accounts domain.AccountRepository,
	journals domain.JournalRepository,
	tx ports.TransactionManager,
	pub ports.EventPublisher,
) *PostJournalHandler {
	return &PostJournalHandler{
		accounts: accounts,
		journals: journals,
		tx:       tx,
		pub:      pub,
...
func (h *PostJournalHandler) Handle(ctx context.Context, cmd PostJournal) (PostJournalResult, error) {
	money, err := domain.NewMoney(cmd.Amount, cmd.Currency)
	if err != nil {
		return PostJournalResult{}, err
	}

	var result PostJournalResult
	err = h.tx.WithinTransaction(ctx, func(ctx context.Context) error {
		...
		if err == nil {
			result = PostJournalResult{JournalID: existing.ID().String(), Created: false}
			return nil
		}
		...
		journal, err := domain.Post(domain.PostParams{
		...
		if err := clearing.ApplyEntry(domain.SideDebit, money, now); err != nil {
			return err
		}
		if err := merchantAcc.ApplyEntry(domain.SideCredit, money, now); err != nil {
			return err
		}
		...
		result = PostJournalResult{JournalID: journal.ID().String(), Created: true}
		return nil
	})
	return result, err
}

func (h *PostJournalHandler) ensureAccount(
	ctx context.Context,
