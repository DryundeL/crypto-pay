...
func main() {
	...
	case "up":
		if *steps > 0 {
			...
			if err := database.RunMigrationsUp(dsn, database.LogIdentity{
	...
	case "version":
		...
		if err != nil {
			...
			log.Printf("⚠ Current migration version: %d (DIRTY - migration may have failed)", ver)
			log.Println("Use -command=force -version=<version> to mark as clean if needed")
		} else {
			log.Printf("✓ Current migration version: %d", ver)
